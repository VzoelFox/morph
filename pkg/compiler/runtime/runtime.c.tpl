...
void mph_gc_mark(void* obj) {
    ...
    // This assumes Array struct layout matches specific offsets? No.
    // We need dynamic marking for Arrays if `type` doesn't cover it.
    // If it's an Array (checked by name/flag?), iterate elements.
    // Or TypeInfo handles it?
    // Current TypeInfo is static offsets. Arrays have dynamic size.
    // We need custom mark for Array.
    // HACK: Check type name.
    if (type->name && strcmp(type->name, "array") == 0) {
        MorphArray* arr = (MorphArray*)obj;
        if (arr->element_type) {
            // It's an array of pointers/objects
            // Wait, arr->data points to the block.
            // Is arr->data marked? If it was alloc'd as 'raw', it's a leaf.
            // If it's pointer array, we need to scan content of arr->data.
            // But arr->data is `void*`.
            // If `element_type` is set, it means elements are pointers to objects?
            // Or elements ARE objects?
            // In C backend: Arrays store pointers `MorphString*` or values `mph_int`.
            // If element_type is not NULL, elements are pointers.
            void** data_ptr = (void**)arr->data;
            // The data block itself is a managed object (leaf).
            ...
            for (size_t i = 0; i < arr->length; i++) {
             ...
             // Array of ints/raw. Mark data block only.
             mph_gc_mark(arr->data);
        }
    }
}
...
void mph_map_resize(MorphContext* ctx, MorphMap* map) {
    ...
    // old_entries will be collected by GC eventually as it's not referenced
}
