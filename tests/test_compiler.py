...
def test_compile_ulangi_statement():
    """Memastikan pernyataan 'ulangi' dikompilasi dengan benar ke IR dengan jumps dan labels."""
    source_code = "ulangi atur x = 1; sebanyak 5 kali"
    ir = compile_code(source_code)

    # Ekspektasi IR:
    # 1. LoadConst 5 -> t1 (jumlah)
    # 2. StoreVar __counter_0 = 0 (inisialisasi counter)
    # 3. Label start_loop_0
    # 4. LoadVar __counter_0 -> t2
    # 5. LoadVar t1 -> t3 (salah, harusnya LoadConst 5 -> t_count)
    # ... ini menjadi rumit. Mari kita sederhanakan pengujian untuk memeriksa komponen utama.

    assert any(isinstance(i, instructions.Label) and 'start_loop' in i.name for i in ir)
    assert any(isinstance(i, instructions.Label) and 'end_loop' in i.name for i in ir)
    assert any(isinstance(i, instructions.Jump) for i in ir)
    assert any(isinstance(i, instructions.JumpIfFalse) for i in ir)

    # Pastikan badan perulangan dikompilasi
    assert any(isinstance(i, instructions.StoreVar) and i.name == 'x' for i in ir)

def test_compile_jika_statement():
    """Memastikan pernyataan 'jika' dikompilasi dengan benar."""
    source_code = "jika benar maka atur x = 1;"
    ir = compile_code(source_code)

    # Harus ada kondisi, lompatan kondisional, dan setidaknya satu label
    assert any(isinstance(i, instructions.JumpIfFalse) for i in ir)
    assert any(isinstance(i, instructions.Label) for i in ir)
    # Pastikan cabang 'then' dikompilasi
    assert any(isinstance(i, instructions.StoreVar) and i.name == 'x' for i in ir)

def test_compile_jika_lainnya_statement():
    """Memastikan pernyataan 'jika-lainnya' dikompilasi dengan benar."""
    source_code = "jika salah maka atur x = 1; lainnya atur y = 2;"
    ir = compile_code(source_code)

    # Harus ada lompatan kondisional dan lompatan tanpa syarat
    assert any(isinstance(i, instructions.JumpIfFalse) for i in ir)
    assert any(isinstance(i, instructions.Jump) for i in ir)
    # Harus ada setidaknya dua label
    assert len([i for i in ir if isinstance(i, instructions.Label)]) >= 2
    # Pastikan kedua cabang dikompilasi
    assert any(isinstance(i, instructions.StoreVar) and i.name == 'x' for i in ir)
    assert any(isinstance(i, instructions.StoreVar) and i.name == 'y' for i in ir)
