...
def test_compile_variable_declaration():
    """Memastikan deklarasi variabel 'atur' dikompilasi dengan benar."""
    ir = compile_code("atur x = 10;")

    # Harusnya ada 2 instruksi: LoadConst untuk 10, dan StoreVar untuk x
    assert len(ir) == 2

    # Instruksi 1: Muat konstanta 10 ke temporary
    assert isinstance(ir[0], instructions.LoadConst)
    assert ir[0].value == 10.0
    assert ir[0].dest == "t1"

    # Instruksi 2: Simpan nilai dari temporary 't1' ke variabel 'x'
...
def test_compile_variable_access():
    ...
    ir = compile_code("atur y = 20; y;")

    assert len(ir) == 3

    # Instruksi 1: Muat 20
    assert isinstance(ir[0], instructions.LoadConst)
    assert ir[0].value == 20.0
    assert ir[0].dest == "t1"

    # Instruksi 2: Simpan ke 'y'
    assert isinstance(ir[1], instructions.StoreVar)
    assert ir[1].name == "y"
    assert ir[1].src == "t1"

    # Instruksi 3: Muat dari 'y'
    assert isinstance(ir[2], instructions.LoadVar)
    assert ir[2].name == "y"
    assert ir[2].dest == "t2"

@pytest.mark.parametrize("source, op_class", [
    ("10 - 4;", instructions.Sub),
    ("7 * 3;", instructions.Mul),
    ("20 / 5;", instructions.Div),
    ("10 > 5;", instructions.GreaterThan),
    ("5 < 10;", instructions.LessThan),
    ("10 == 10;", instructions.Equals),
    ("10 != 5;", instructions.NotEquals),
    ("10 <= 10;", instructions.LessThanOrEqual),
    ("10 >= 10;", instructions.GreaterThanOrEqual),
])
def test_compile_all_binary_operators(source, op_class):
    """Memastikan semua operator biner dikompilasi dengan benar."""
    ir = compile_code(source)

    assert len(ir) == 3

    # Periksa apakah instruksi terakhir adalah operasi biner yang benar
    assert isinstance(ir[2], op_class)
    assert ir[2].left == "t1"
    assert ir[2].right == "t2"
    assert ir[2].dest == "t3"
